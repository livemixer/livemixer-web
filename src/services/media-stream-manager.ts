...
class MediaStreamManagerImpl {
  ...
  notifyStreamChange(itemId: string): void {
    this.listeners.get(itemId)?.forEach((cb) => {
      try {
        ...
        console.error(
          '[MediaStreamManager] Error in stream change callback:',
          err,
        );
      }
    });
    ...
    try {
      ...
      if (!hasLabels && videoDevices.length > 0) {
        ...
        try {
          ...
          console.warn(
            '[MediaStreamManager] Could not get camera permission:',
            permErr,
          );
        }
      }
  ...
  async getAudioInputDevices(): Promise<MediaDeviceInfo[]> {
    try {
      ...
      const needsPermission =
        (audioDevices.length > 0 && !hasLabels) || audioDevices.length === 0;
      ...
      if (needsPermission) {
        try {
          ...
          console.warn(
            '[MediaStreamManager] Could not get microphone permission:',
            permErr,
          );
        }
      }
  ...
  async getAudioOutputDevices(): Promise<MediaDeviceInfo[]> {
    try {
      ...
      console.error(
        '[MediaStreamManager] Error getting audio output devices:',
        err,
      );
      return [];
    }
