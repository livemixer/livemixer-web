...
export class PluginContextManager {
     ...
     */
    updateState(updater: Partial<PluginContextState> | ((state: PluginContextState) => Partial<PluginContextState>)): void {
        ...
        this.state = {
            ...this.state,
            ...updates,
            scene: updates.scene ? { ...this.state.scene, ...updates.scene } : this.state.scene,
            playback: updates.playback ? { ...this.state.playback, ...updates.playback } : this.state.playback,
            output: updates.output ? { ...this.state.output, ...updates.output } : this.state.output,
            ui: updates.ui ? { ...this.state.ui, ...updates.ui } : this.state.ui,
            devices: updates.devices ? { ...this.state.devices, ...updates.devices } : this.state.devices,
            user: updates.user ? { ...this.state.user, ...updates.user } : this.state.user,
        };
    }
        ...
        if (callbacks) {
            callbacks.forEach(cb => {
                try {
                    ...
                    console.error(`[PluginContext] Error in event listener for ${event}:`, err);
                }
            });
        }
    }
    ...
    createContextForPlugin(
        ...
        const context: IPluginContext = {
            ...
            // Event subscription
            subscribe: <E extends PluginContextEvent>(event: E, callback: EventCallback<E>) => {
            ...
            requestPermission: async (requestedPermissions: PluginPermission[]) => {
                ...
                logger.warn(`Permission request not implemented for non-builtin plugins`);
                return false;
            },
    ...
    disposePlugin(pluginId: string): void {
        ...
        for (const [slot, contents] of this.slots) {
            this.slots.set(slot, contents.filter(c => c.pluginId !== pluginId));
        }
        this.notifySlotChange();
    ...
    private createReadonlyStateProxy(): PluginContextState {
        ...
        const createDeepReadonlyProxy = <T extends object>(obj: T): T => {
            return new Proxy(obj, {
                ...
                set() {
                    console.warn('[PluginContext] Cannot directly modify state. Use actions instead.');
                    return false;
                },
                deleteProperty() {
    ...
    private createActionsForPlugin(
        ...
        const hasPermission = (p: PluginPermission) => permissions.granted.includes(p);
        const logger = this.createScopedLogger(pluginId);
            ...
            scene: {
                ...
                duplicateItem: async (itemId) => {
                    ...
                    if (!handlers.scene?.duplicateItem) {
                        throw new Error('Action handler not configured: scene.duplicateItem');
                    }
