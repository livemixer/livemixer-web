...
function isLegacyProps(props: AudioInputDialogProps): props is LegacyAudioInputDialogProps {
        ...
        <div className="flex items-end gap-0.5 h-8">
            {Array.from({ length: bars }).map((_, i) => {
                ...
                const color = i < bars * 0.6
                    ? (active ? 'bg-green-500' : 'bg-green-900/40')
                    : i < bars * 0.85
                        ? (active ? 'bg-yellow-500' : 'bg-yellow-900/40')
                        : (active ? 'bg-red-500' : 'bg-red-900/40');
                return (
                    <div
                        key={i}
...
export function AudioInputDialog(props: AudioInputDialogProps) {
        ...
        const loadDevices = async () => {
            try {
                ...
                if (audioDevices.length === 0 && audioTrack) {
                    console.log('No devices from enumerateDevices, using fallback from stream track');
    ...
    const handleConfirm = () => {
        ...
        if (isLegacyProps(props)) {
            ...
            setPendingAudioInputStream({ stream: previewStream, deviceId: selectedDeviceId, label });
            setPreviewStream(null);
            ...
            <DialogContent className="bg-linear-to-b from-neutral-850 to-neutral-900 border-neutral-700/50 text-white max-w-lg">
                ...
                <div className="space-y-4 mt-4">
                    ...
                    <div>
                        ...
                        >
                            {devices.length === 0 ? (
                                <option value="">{t('plugins.io.livemixer.audioinput.dialog.noDevices')}</option>
                            ) : (
                                devices.map(device => (
                        ...
                        <div className="p-4 bg-[#1a1a2e] border border-[#3e3e42] rounded-lg">
                            {isLoading && (
                                <div className="flex items-center justify-center gap-2 py-2">
                                    ...
                                    <span className="text-sm text-gray-400">{t('plugins.io.livemixer.audioinput.dialog.loading')}</span>
                                </div>
                            )}
                            ...
                            {!isLoading && !error && !previewStream && (
                                <div className="flex items-center justify-center gap-2 py-2 text-gray-500">
                                    ...
                                    <span className="text-sm">{t('plugins.io.livemixer.audioinput.dialog.selectToPreview')}</span>
                                </div>
                            )}
