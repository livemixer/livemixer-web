...
export const audioStreamCache = {
    ...
    set: (itemId: string, data: { stream: MediaStream; deviceId?: string; label?: string }) => {
...
export function setPendingAudioInputStream(data: { stream: MediaStream; deviceId: string; label: string } | null) {
    if (data) {
            ...
            en: {
                'plugins.io.livemixer.audioinput': {
                    ...
                    dialog: {
                        ...
                        description: 'Select a microphone and check the audio level before adding',
                        device: 'Device',
    ...
    render: (commonProps: any) => {
        ...
        const startCapture = useCallback(async (targetDeviceId?: string) => {
            try {
                ...
                audioStreamCache.set(streamIdRef.current, { stream, deviceId: actualDeviceId, label });
                setIsCapturing(true);
        ...
        useEffect(() => {
            ...
            checkCache();
            const unsubscribe = onAudioStreamCacheChange(streamIdRef.current, checkCache);
            return () => unsubscribe();
        ...
        const levelBarWidth = isCapturing ? Math.max(4, (audioLevel / 100) * (w - 20)) : 0;
            ...
            <>
                ...
                {isCapturing && (
                    <Rect
                        ...
                        width={levelBarWidth}
                        height={4}
                        fill={audioLevel > 85 ? '#ef4444' : audioLevel > 60 ? '#f59e0b' : '#22c55e'}
                        cornerRadius={2}
                        listening={false}
                    />
                )}
