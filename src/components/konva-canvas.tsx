...
export const KonvaCanvas = forwardRef<KonvaCanvasHandle, KonvaCanvasProps>(function KonvaCanvas(
  ...
  const syncToOutputCanvas = useCallback(() => {
    ...
    try {
      ...
      const renderPixelRatio = Math.max(outW / displayWidth, outH / displayHeight);
      ...
      if (currentScene) {
        ...
        for (const item of currentScene.items) {
          if (item.type !== 'livekit_stream' || item.visible === false) continue;
  ...
  const handleTransformEnd = useCallback(
    (itemId: string, currentTransform: Transform | undefined, e: Konva.KonvaEventObject<Event>) => {
      const node = e.target;
      ...
      {stageSize.width > 0 && stageSize.height > 0 && (
        <div
          ...
          style={{
            ...
            boxShadow: '0 2px 4px -1px rgba(0, 0, 0, 0.4), 0 1px 2px -1px rgba(0, 0, 0, 0.25)',
            overflow: 'hidden',
            position: 'relative',
          }}
        >
                ...
                <div
                  ...
                  ref={el => {
                    if (el) {
                      ...
                      if (video) livekitVideoMapRef.current.set(item.id, video);
                    } else {
