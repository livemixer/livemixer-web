...
export function PluginContextProvider({ children }: PluginContextProviderProps) {
    ...
    const getContext = useCallback((pluginId: string): IPluginContext | null => {
...
export function usePluginContextSystem(): PluginContextValue {
    ...
    if (!context) {
        throw new Error('usePluginContextSystem must be used within a PluginContextProvider');
    }
    return context;
}
    ...
    const visibleContents = useMemo(() => {
        return contents.filter(content => {
            ...
            try {
                ...
                console.error(`[Slot] Error in visibility check for ${content.id}:`, err);
                return false;
            }
        });
    }, [contents, state]);
 ...
 */
export function DialogSlot({ activeDialog, dialogProps, onClose }: DialogSlotProps) {
