...
function PluginRow({ plugin }: { plugin: ISourcePlugin }) {
  ...
  return (
    <div className="border-b border-[var(--lm-border)] last:border-b-0">
      <button
        type="button"
        onClick={() => setExpanded(!expanded)}
        className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-[var(--lm-hover)] transition-colors text-left"
      >
        <span className="text-[var(--lm-muted-2)] shrink-0">
          {expanded ? (
      ...
      {expanded && (
        <div className="px-3 pb-3 pl-12 space-y-2">
          ...
          {plugin.propsSchema && (
            <div className="text-xs">
              <span className="text-[var(--lm-muted-2)]">Props: </span>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[560px]">
        ...
        <div className="border border-[var(--lm-border)] rounded-lg overflow-hidden max-h-[400px] overflow-y-auto">
          {plugins.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10">
              <Puzzle className="w-8 h-8 text-[var(--lm-muted-2)] mb-2" />
              <span className="text-sm text-[var(--lm-muted-2)]">
        ...
        {/* Summary */}
        {plugins.length > 0 && (
          <div className="flex items-center justify-between text-xs text-[var(--lm-muted-2)] pt-1">
            <span>
