...
export function MainLayout({
  ...
  const { contentRef, bottomBarHeight, maxBottomBarHeight, handlePointerDown } =
    useResizablePanel();
  return (
    <div className="flex flex-col w-full h-full bg-linear-to-b from-neutral-900 via-neutral-850 to-neutral-950 text-white overflow-hidden">
      ...
      <div ref={contentRef} className="flex-1 min-h-0 flex flex-col">
        ...
        {bottomBar && (
          <div
            className="relative shrink-0 bg-neutral-900/80 border-t border-neutral-700/50 flex overflow-hidden backdrop-blur-sm shadow-lg"
            style={{ height: Math.min(bottomBarHeight, maxBottomBarHeight) }}
          >
            <div
              className="absolute inset-x-0 -top-2 h-4 cursor-row-resize z-20"
              onPointerDown={handlePointerDown}
              role="separator"
              aria-label="Resize bottom bar"
            >
              <div className="mx-auto mt-2 h-1 w-16 rounded-full bg-neutral-600/70" />
            </div>
            {bottomBar}
          </div>
        )}
